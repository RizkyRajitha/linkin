...
export async function restoreBackupData(data) {
  ...
  // get old data as temporal backup
  let oldData = await getPageDatawLinkAndSocialData();

  const deleteOldData = async () => {
    await Promise.all([
      Prisma.linkdata.deleteMany(),
      Prisma.socialdata.deleteMany(),
      Prisma.pagedata.deleteMany(),
      Prisma.$executeRaw("ALTER SEQUENCE linkdata_id_seq RESTART WITH 1;"),
  ...
  let { pageData, linkData, socialData } = data;
  try {
    await Prisma.pagedata.create({ data: pageData });
    await Prisma.linkdata.createMany({ data: linkData });
    await Prisma.socialdata.createMany({ data: socialData });
    return;
  } catch (error) {
    // try to recover previous data from backup

    // delete previous data (possibly created in try statement)
    await deleteOldData();

    let { pageData, linkData, socialData } = oldData;
    await Prisma.pagedata.create({ data: pageData });
    await Prisma.linkdata.createMany({ data: linkData });
    await Prisma.socialdata.createMany({ data: socialData });
    throw new Error(error.message);
